...
def execCommand(command):
	...
	if command["subcommand"] == "display":
		...
		# Handles the displaying of specific months.
		...
		# Handles the displaying of specific years.
		...
		if add is not None:
			...
			# I couldn't think of a better way to display the current month screen again, so this recursion.
			execCommand(parseCommand("display -m"))

		# Handles the searching of specific dates for events.
		...
		# Handles the deletion of specific events from dates.
			...
			if date in EVENTS.keys():
				try:
					index = int(delete[1])
					# Check if the event at the specified index exists.
					if not 0 < index < len(EVENTS[date]) + 1:
						renderError(CMD_WIDTH, CMD_HEIGHT, "Your index for this day must be in between 0 and " + str(len(EVENTS[date]) + 1))
						return None
				except:
					renderError(CMD_WIDTH, CMD_HEIGHT, "Please enter a valid numerical character.")
					return None
				# Removes the event, and if it was the only event for that date, removes the whole date from the dictionary.
	...
	# Handles closing the application safely.
